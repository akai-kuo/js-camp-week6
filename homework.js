...
3. 什麼是 RESTful API？
   答：RESTful API 是一種遵循 REST（Representational State Transfer）設計原則的 API，它使用標準的 HTTP 方法（GET、POST、PATCH、PUT、DELETE）來操作資源，並通過 URL 來識別資源。
...
if (require.main === module) {
  async function runTests() {
    ...
    try {
      ...
      console.log("getProductsSafe:", result?.success ? "成功" : result?.error || "回傳 undefined");
    } catch (error) {
